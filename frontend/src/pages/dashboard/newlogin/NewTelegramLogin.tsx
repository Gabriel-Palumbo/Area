...
const NewTelegramLogin = () => {
    ...
    const handleActivate = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${serverUrl}/services/telegram/save_sender`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            if (response.ok) {
                window.alert("Telegram service activated successfully");
                navigate('/newarea/telegram');
            } else {
                window.alert("Failed to activate telegram service");
            }
        } catch (error) {
            console.error('Error activating telegram service:', error);
        }
    };
    ...
    return (
        <div className={styles.newArea}>
            <div className={styles.header}>
                <img src={arrowLeft} alt="Retour" className={styles.backArrow} onClick={() => navigate('/newarea/telegram')}/>
                <h1>telegram</h1>
            </div>
...
export default NewTelegramLogin;