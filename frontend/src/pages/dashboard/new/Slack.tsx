...
const Slack_new = () => {
    ...
    useEffect(() => {
        const slackModule = Object.values(modules).find(module => module.name === 'slack');
        if (slackModule) {
            setIcon(slackModule.url);
        }
    }, [modules]);
    ...
    return (
        <div className={styles.newArea}>
            ...
            <p className={styles.description}>Get the latest slack information from your area and way more !</p>
            <button className={styles.button} onClick={() => navigate('/newarea/slack/newslack')}>Activer slack</button>
        </div>
    );
};

export default Slack_new;
